...
from dust3r.utils.image import load_images_tensor
...
class Du3d3d:
    ...
    def render_pcd(self,pts3d,imgs,masks,views,renderer,device,nbv=False):
        ...
        col = col.float().to(device)  #
                
        point_cloud = Pointclouds(points=[pts], features=[col]).extend(views)
        print("point_cloudpoint_cloudpoint_cloudpoint_cloudpoint_cloud",type(point_cloud))  
        #points = point_cloud.points_padded()  # 
        #features = point_cloud.features_padded()  # 

        #points_float32 = points.to(torch.float32)
        #features_float32 = features.to(torch.float32)

        #point_cloud = Pointclouds(points=points_float32, features=features_float32)      
    ...
    def run_render(self, pcd, imgs,masks, H, W, camera_traj,num_views,nbv=False):
        ...
        if isinstance(pcd, list):
            for i, tensor in enumerate(pcd):
                if isinstance(tensor, torch.Tensor):
                    print(f"pcd[{i}] dtype: {tensor.dtype}")
                else:
                    print(f"pcd[{i}] is not a tensor, it is of type {type(tensor)}")
        else:
            print(f"pcd dtype: {pcd.dtype}")
        
        # 
        if isinstance(imgs, torch.Tensor):
            print(f"imgs dtype: {imgs.dtype}")
        else:
            print(f"imgs is of type {type(imgs)}")
    
        if isinstance(masks, torch.Tensor):
            print(f"masks dtype: {masks.dtype}")
        else:
            print(f"masks is of type {type(masks)}")                
    ...
    def run_diffusion(self, renderings):
        ...
        print("videos",videos.shape) #torch.Size([1, 3, 25, 576, 1024])
        #print(a)
        condition_index = [0]
    ...
    def nvs_single_view(self, gradio=False):
        ...
        render_results[0] = self.img_ori   ##第0帧是真实值
    ...
    def nvs_sparse_view_interp(self,image_dir):
        #print("self.opts.image_dir",self.opts.image_dir)
        self.images, self.img_ori = self.load_initial_dir(image_dir=image_dir)
        print(type(self.images[0]),type(self.img_ori[0]))
        print(self.images[0],len(self.img_ori))
        print(self.img_ori[0].shape)
        ...
        print("render_results",type(render_results))
    ...
    def setup_dust3r(self):
        with torch.no_grad():
            print("self.opts.model_path,self.opts.model_path,self.opts.model_path,self.opts.model_path,",self.opts.model_path)
            self.dust3r = load_model(self.opts.model_path, self.device)
            self.dust3r = self.dust3r.float()
            self.dust3r.eval()  #
    ...
    def load_initial_dir(self, image_dir):
        ...
        #print("image_files 99999999999",image_files )
        ###################################################################################        
        #image_files=torch.rand(1, 3, 2, 576, 1024)
        image_files=image_dir
        images = load_images_tensor(image_files, size=512,force_1024 = True)
