...
def _preprocess_bbox(bbox_mode, canvas_size, examples, is_train=True,
    ...
    instance_token: List[torch.Tensor] = [
    ...
    for idx in range(B):
        ...
        instance_kf = instance_token[idx]
    ...
    ret_instance_token = -torch.ones(B, N_out, max_len, dtype=torch.long)
    for _b in range(B):
        _bboxes = bboxes[_b]
        _classes = classes[_b]
        _instance_token = instance_token[_b]
        for _n in range(N_out):
            ...
            ret_instance_token[_b, _n, :this_box_num] = _instance_token[_n]
    ...
    ret_dict = {
        ...
        "classes": ret_classes,
        "masks": ret_masks,
        "instance_token_3d": ret_instance_token
    }
